...
df.info()
df_answer = df.price.groupby(by=df.waterfront).mean()
abs(df_answer[0]-df_answer[1])
...
aa = corr.price.iloc[1:]
aa.nlargest()
 
#answer sqft_living / yr_built
...
dfdf = df.drop(['price','id', 'date', 'zipcode'], axis = 1)
y = df.price
...
pd.crosstab(df.State, columns=df[x_var], values = df.Profit, aggfunc='mean')

temp = df[df.State == 'New York']
...
df = df[(df['previous_owners']==1) & (df['engine_power']==51)]

df_mean = df['km'].groupby(by = df.model).mean()
day_mean = df['age_in_days'].groupby(by = df.model).mean()
df_mean = df_mean.reset_index()
day_mean = day_mean.reset_index()
day_mean
df_mean

dfdf = pd.merge(df_mean, day_mean, how='inner')

dfdf['answer'] = dfdf.km/dfdf.age_in_days
dfdf['answer'][2]/dfdf['answer'][0] * 100
...
x = pd.DataFrame(df[x_var])


model = OLS(y, x).fit()
model.summary()

sample = {'model' : 'pop','engine_power' : [51],'age_in_days' : [400],'km' : [9500],'previous_owners':[2]}
sample = [51, 400, 9500]
sample = pd.DataFrame(sample)
sample = sample[x_var]
sample = add_constant(sample)
